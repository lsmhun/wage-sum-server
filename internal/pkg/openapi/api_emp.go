...
func (c *EmpApiController) FindEmpsByMgrId(w http.ResponseWriter, r *http.Request) {
	query := r.URL.Query()
	mgrIdParam, err := parseInt64Parameter(query.Get("mgrId"), true)
	if err != nil {
		c.errorHandler(w, r, &ParsingError{Err: err}, nil)
		return
	}
	result, err := c.service.FindEmpsByMgrId(r.Context(), mgrIdParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
